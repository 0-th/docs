...
Learn how to publish files to IPFS and Filecoin via Storacha from a Continuous Integration server.
...
Reference our [Quickstart Guide](/quickstart/) to install and log in `w3cli`.
...
If you'd like to learn more or troubleshoot a space, please check out our [Create a Space guide](/how-to/create-space/#using-the-cli).
...
On your local machine, use [w3cli[(/w3cli/) to delegate capabilties to upload to our space to the public DID for the signing key we created.
...
Set `W3_PRINCIPAL=<the signing key>` in the CI environment. The `w3` commmand will use the value as the signing key to use. Check out [this](https://github.com/storacha/w3cli?tab=readme-ov-file#w3_principal) to read more.
...
The [add-to-web3][https://github.com/marketplace/actions/add-to-web3] action is a lightweight wrapper around [w3cli][/w3cli/]. You create the key and proof as before, and the action configures and runs `w3` in CI for you.
...
We use the action to publish _this website_ (👁️⁂👁️) to IPFS when a PR is merged to `main`. We use the `cid` output from the action as input to [update a dns record](https://github.com/storacha/www/blob/18044a3814846967fca1126ab3baa3973c43e908/.github/workflows/deploy.yml#L79-L89) called a [DNSlink](https:/dnslink.io) for the domain.

Setting the [DNSLink](https:/dnslink.io) announces the new root CID for the website, so [IPFS-aware browsers](https://brave.com/ipfs-support/) and [IPFS Companion](https://docs.ipfs.tech/install/ipfs-companion/) can load the site over IPFS.
